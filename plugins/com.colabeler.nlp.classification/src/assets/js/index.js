...
document.addEventListener("actor-will-enter",function(ev){
    var doc = ev.detail;
    var annotation = doc.outputs.annotation;
    $(".preview > pre").text(doc.content);
    var labelInputs = $("input");
    labelInputs.val("");//清空
    for(var ri in annotation){
        labelInputs.eq(ri).val(annotation[ri].class.value);
    }
})
document.addEventListener("actor-will-finish",function(ev){
    ...
    for(let i = 0;i<labelInputs.length;i++){
        ...
        doc.outputs.annotation.push({"class":{"id":i+1,"value":targetValue}});
    }
})